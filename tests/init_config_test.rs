use scroll_proving_sdk::prover::CircuitType;
...
fn test_config_without_envs() {
    ...
    temp_env::with_vars(
        ...
        || {
            ...
            let cfg = CloudProverConfig::from_file_and_env(default_config_path.to_string())
                .expect("Issue loading test configuration file");

            // Check a few values for consistency with test_data file
            assert_eq!(cfg.sdk_config.prover_name_prefix, "sindri_");
            assert_eq!(cfg.sdk_config.prover.circuit_types[0], CircuitType::Chunk);
            assert_eq!(cfg.sdk_config.prover.circuit_types[1], CircuitType::Batch);
            assert_eq!(cfg.sdk_config.prover.circuit_types[2], CircuitType::Bundle);
            assert_eq!(cfg.sdk_config.db_path, Some("db".to_string()));
            assert_eq!(cfg.sdk_config.prover.n_workers, 1_usize);
        },
    );
}
...
fn test_config_with_envs() {
    ...
    temp_env::with_vars(
        ...
        || {
            ...
            assert_eq!(
                cfg.sdk_config.prover.n_workers,
            ...
            assert_eq!(
                cfg.sdk_config.coordinator.base_url, coordinator_url_override,
...
async fn test_initialize_from_config_pipeline() {
    ...
    let _cloud_prover = CloudProver::new(cfg);
}
